...
> extends DatabaseBaseRepositoryAbstract<EntityDocument> {
    ...
    async findAllDistinct<T = Entity>(
        ...
        const findAll = this._repository.distinct<any>(fieldDistinct, find);
    ...
    async exists(
        ...
        if (options?.withDeleted) {
            ...
            exist.where(DATABASE_IS_DELETED_FIELD_NAME, false);
    ...
    async restoreManyByIds(
        ...
        const rest = this._repository
            ...
            .where(DATABASE_IS_DELETED_FIELD_NAME, true)
            .where(DATABASE_DELETED_AT_FIELD_NAME)
            .exists(true);
