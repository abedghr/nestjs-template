...
> extends DatabaseBaseRepositoryAbstract<EntityDocument> {
    ...
    async findAllDistinct<T = EntityDocument>(
        ...
        const findAll = this._repository.distinct<any>(
            fieldDistinct,
            find
        );
    ...
    async raw<RawResponse, RawQuery = PipelineStage[]>(
        ...
        if (options?.order) {
            ...
            Object.keys(options.order).forEach((field) => {
                let sortValue = ENUM_PAGINATION_ORDER_DIRECTION_NUMBER_FORMAT_TYPE.ASC;

                if (options.order[field] === ENUM_PAGINATION_ORDER_DIRECTION_TYPE.DESC) {
                    sortValue = ENUM_PAGINATION_ORDER_DIRECTION_NUMBER_FORMAT_TYPE.DESC;
                }
                sortFields[field] = sortValue;
            });
